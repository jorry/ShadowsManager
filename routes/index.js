...
router.get('/', function (req, res) {
    console.log('loginjorry', 'loginjorry');
    res.redirect('/loginjorry');
});

//跳转搜索页
router.get('/tomain', function (req, res,next) {
    console.log('./layouts/layout', 'layout');
    res.render('search', { layout: 'layout' });
});


//a/b testting
router.get('/abtestting', function (req, res) {
    console.log('打印参数', 'abtestting');
    res.render('abtestting', {
        title: 'shadows-a/b Testting',
        arr: [{sch: '', ab: 'abs',lib: '', abt: '', log: ''}]
    });
});

//获取书目检索页
router.get('/search', function (req, res) {
    console.log('打印参数', 'search');
    res.render('search', {
        title: '书目检索-图书流通管理系统',
        arr: [{sch: 'active',ab:'', lib: '', abt: '', log: ''}]
    });
});
...
router.get('/search/r', function (req, res, next) {
    ...
    Book.findBooksByTitle(content, function (err, books) {
        ...
        res.render('result', {
            title: '搜索结果-图书流通管理系统',
            arr: [{sch: 'active',ab:'', lib: '', abt: '', log: ''}], books: books
        });
    });
});
...
router.get('/borrow', ensureAuthenticated, function (req, res, next) {
    ...
    Book.findBookBybarcode(barcode, function (err, book) {
        ...
        res.render('borrow', {
            title: '借阅-图书流通管理系统',
            arr: [{sch: 'active',ab:'', lib: '', abt: '', log: ''}], book: book
        });
    });
});
...
router.get('/order', ensureAuthenticated, function (req, res, next) {
    ...
    Book.findBooksByISBN(isbn, function (err, bookInfo, booksState) {
        ...
        res.render('order', {
            title: '预约-图书流通管理系统',
            arr: [{sch: 'active',ab:'', lib: '', abt: '', log: ''}], bookInfo: bookInfo
        });
    });
})
